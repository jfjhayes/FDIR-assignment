...
figure;
figure;
subplot(3,1,1); plot(tout, rad2deg(xout(:,5))); 
ylabel('\psi (deg)'); title('Yaw Angle Over Time'); grid on;

subplot(3,1,2); stairs(tout, rad2deg(uout(:,2)));  
ylabel('\delta_r (deg)'); title('Rudder Deflection Over Time'); grid on;

subplot(3,1,3); plot(tout, rad2deg(xout(:,2)));  
ylabel('r (deg/s)'); title('Yaw Rate Over Time'); grid on;